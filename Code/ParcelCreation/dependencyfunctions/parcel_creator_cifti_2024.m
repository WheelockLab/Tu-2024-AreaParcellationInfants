function parcel_creator_cifti_2024(edgeciftiname,filestem,threshperc,outputtemplate,edgevalthreshperc,minparcelsize)
% modified from parcel_creator_cifti so that the thresholds can be function
% arguments
...
% be applied to other data that does not have a medial wall). 
%
...
edgemetric = edge_cifti.data; edge_cifti.data = [];
...
sortedcorticalmetric = sortedcorticalmetric(~isnan(sortedcorticalmetric));
minimathresh = sortedcorticalmetric(round(numel(sortedcorticalmetric)*minimathreshperc));
if edgevalthreshperc>1
    edgevalthresh = 1;
else
...
if exist('outputtemplate','var') && ~isempty(outputtemplate)
    %read in output space
    
    if isstruct(outputtemplate)
        cifti_template = outputtemplate;
    else 
        cifti_template = ft_read_cifti_mod(outputtemplate);
    end
    
    %make the variable that will become the translated data
    newlabel = zeros(size(cifti_template.data,1),1);
    ...
    %put the data onto a space that has all surface vertices
    allsurface_edgedata = zeros(nsurfverts,1);
    allsurface_edgedata(edge_cifti.brainstructure(1:nsurfverts) > 0) = label(1:nnz(edge_cifti.brainstructure(1:nsurfverts) > 0));
    
    %remove the vertices that the output space does not have
    ...
    newlabel(1:length(cifti_template_surface_edgedata)) = cifti_template_surface_edgedata;
    ...
    if isfield(edge_cifti,'dim') && isfield(cifti_template,'dim')
        ...
        for templateind = template_subcort_inds(:)'
            ...
            if ~isempty(matching_edgeind)
                ...
                newlabel(template_dataind) = label(matching_edge_dataind);
            end
        end
...
%         newlabel((nnz(cifti_template.brainstructure==1) + nnz(cifti_template.brainstructure==2) + 1):end) = [];
    end
    label = newlabel;

else
    cifti_template = edge_cifti;
end
...
cifti_template.data(1:length(label),1) = label;

if edgevalthresh==1
    parcels_path = [filestem '_global_edgethresh_' num2str(round(threshperc*100)/100),'_nogap_minsize_',num2str(minparcelsize),'_relabelled.dtseries.nii'];
else
    parcels_path = [filestem '_global_edgethresh_' num2str(round(threshperc*100)/100),'_heightperc_',num2str(round(edgevalthreshperc*100)/100),'_minsize_',num2str(minparcelsize),'_relabelled.dtseries.nii'];
end
ft_write_cifti_mod(parcels_path,cifti_template);

numparcels = nnz(unique(label));
disp(['Final number of parcels: ' num2str(numparcels)])

%convert to dlabel
wb_command = '/data/cn/data1/linux/bin/wb_command';
input_cifti = parcels_path;
output_cifti = strrep(parcels_path,'dtseries','dlabel');
label_list = '""';
command = [wb_command,' -cifti-label-import ',input_cifti,' ',label_list,' ',output_cifti];
...
%RELABEL Relabel a parcellation. Just a utility function. From Arslan 2018
