function [r,c] = paircorr_mod_2024(a,b)
% modified from paircorr_mod.m to take care of the NaN values in the data
% when there is not signal coverage.

% N.B. this only takes care of off diagonal NaNs where the whole row is NaN
% not for the diagonal NaNs
...
mag_a = sqrt(nansum(a.^2, 1));
mag_b = sqrt(nansum(b.^2, 1));

notnan = ~isnan(a(:,1));
tmp = (a(notnan,:)' * b(notnan,:));
r = tmp ./ (mag_a' * mag_b);
c = tmp/(sum(notnan)-1);
